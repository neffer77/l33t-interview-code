...
const partial=L.feedbackModel({buildingId:'lab',building:{name:'Graph Lab'},unlocked:false,beforeNeed:20,afterNeed:12,progress:8,contract:{next:{id:'research'}}});
ok(partial.kind==='progress'&&partial.percent===40,'visible feedback should quantify partial contract progress');
state.unlocked=true;
...
ok(result.feedback?.buildReady&&result.feedback?.percent===100,'unlock result should expose build-ready visual feedback');
ok(state.learningCity.pending?.buildingId==='lab','city consequence should be persisted for UI feedback');
ok(C.visible?.buildingId==='lab','city catalog should receive the visible solve consequence');
ok(emitted.some(([n])=>n==='learning-city:building-unlocked'),'unlock should emit a city-facing event');
ok(emitted.some(([n])=>n==='learning-city:visible-consequence'),'solve should emit a dedicated visible-consequence event');
...
console.log('Learning city loop passed: targeted training now produces quantified, build-ready, city-visible consequences.');
