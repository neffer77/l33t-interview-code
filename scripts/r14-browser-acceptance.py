...
def wait_city(page):
    ...
    switch_city(page)
    try: page.wait_for_function("""() => {const C=window.Codeopolis,s=C?.phaserCity?.game?.scene?.getScene?.('CodeopolisCity'),h=C?.phaserCity?.host;return !!s?.sys?.isActive?.()&&!!h&&getComputedStyle(h).display!=='none'&&h.getBoundingClientRect().width>1&&h.getBoundingClientRect().height>1}""",timeout=12000)
...
def run_viewport(browser,base_url,out_dir,name,width,height,deployed=False):
    ...
    page.evaluate("""()=>{const C=Codeopolis,s=C.game?.state||window.state;C.ConceptResources.award(s,{challenge:{district:'arrays',difficulty:'easy',pattern:'Foundations'},concept:'R14 starter',rewardOverride:{resourceId:'materials',amount:12}});s.money=Math.max(5000,Number(s.money)||0)}""");switch_city(page);page.wait_for_function("()=>(Codeopolis.game?.state||window.state)?.r4Construction?.stage==='build'",timeout=10000);wait_city(page);page.locator('#phaserCityHost').screenshot(path=str(vp_dir/'03-build-ready.png'))
