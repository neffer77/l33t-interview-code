...
def audit(page):
    return page.evaluate('() => window.Codeopolis.R14PlayerAcceptance.audit()')

def canvas_point(page, x, y):
    return page.evaluate("""([x,y]) => {
      const C=window.Codeopolis,s=C.phaserCity.game.scene.getScene('CodeopolisCity'),c=s.cameras.main,p=s.toWorld(x,y),r=s.game.canvas.getBoundingClientRect();
      return {x:r.left+(p.x-c.scrollX)*c.zoom,y:r.top+(p.y-c.scrollY)*c.zoom};
    }""", [x,y])

def manual_first_build(page):
    page.evaluate("""() => {
      const C=window.Codeopolis,s=C.game?.state||window.state;
      s.money=Math.max(5000,Number(s.money)||0);
      s.buildings=[...new Set([...(s.buildings||[]),'house','market','foundry','solar','park'])];
      C.phaserCity.catalog?.render?.();
      C.phaserCity.catalog?.acquire?.('house');
    }""")
    page.wait_for_function("() => window.Codeopolis.game.world.world.tool?.mode==='building'", timeout=5000)
    pt=canvas_point(page,5,5)
    page.mouse.click(pt['x'],pt['y'])
    page.wait_for_function("() => window.Codeopolis.game.world.placedBuildings().length===1", timeout=8000)

def seed_operating_city(page):
    return page.evaluate("""() => {
      const C=window.Codeopolis,s=C.game?.state||window.state,w=C.game?.world;
      s.money=Math.max(5000,Number(s.money)||0);s.population=Math.max(12,Number(s.population)||0);s.eraLevel=3;
      s.ageProgression=Object.assign({},s.ageProgression,{level:3});
      s.tech=[...new Set([...(s.tech||[]),'arrays','maps','search','energy','graphs'])];
      s.buildings=[...new Set([...(s.buildings||[]),'market','foundry','solar','park'])];
      const placements=[['market',8,2],['foundry',2,5],['solar',8,5],['park',5,2]],results=[];
      ...
      for(let x=1;x<=10;x++)if(!w.tile(x,4)?.buildingId&&!w.tile(x,4)?.occupiedBy)w.setRoad(x,4,true);
      for(const [x,y] of [[2,3],[5,3],[8,3],[2,4],[5,4],[8,4]])if(!w.tile(x,y)?.buildingId&&!w.tile(x,y)?.occupiedBy)w.setRoad(x,y,true);
      C.BuildingOperations?.sync?.(s,w);C.PopulationSimulation?.step?.(s,w,1);
      C.phaserCity?.game?.scene?.getScene?.('CodeopolisCity')?.refresh?.();C.R14PlayerAcceptance?.sync?.();
...
def run_viewport(browser, base_url, out_dir, name, width, height, deployed=False):
    ...
    page.goto(url,wait_until='domcontentloaded',timeout=90000);page.wait_for_selector('body',timeout=15000);switch_city(page);wait_city(page)
    ...
    page.evaluate("""() => {const C=window.Codeopolis,s=C.game?.state||window.state;C.events.emit('learning:resource-earned',{resourceId:'materials',amount:12,source:'r14-browser-acceptance'});s.money=Math.max(5000,Number(s.money)||0)}""")
    switch_city(page);page.wait_for_function("() => (window.Codeopolis.game?.state||window.state)?.r4Construction?.stage==='build'",timeout=10000);page.wait_for_timeout(350)
    page.locator('#phaserCityHost').screenshot(path=str(vp_dir/'03-build-ready.png'))
    ...
    page.locator('#phaserCityHost').screenshot(path=str(vp_dir/'04-operating-city.png'))

    page.wait_for_selector('.r13-campaign-fab',state='visible',timeout=25000);page.locator('.r13-campaign-fab').click();page.wait_for_selector('.r13-campaign-panel.show',timeout=5000)
    page.locator('#phaserCityHost').screenshot(path=str(vp_dir/'05-interview-campaign.png'))
    page.wait_for_selector('.r12-custom-fab',state='visible',timeout=15000);page.locator('.r12-custom-fab').click();page.wait_for_selector('.r12-custom-panel.show',timeout=5000)
    if page.locator('.r13-campaign-panel.show').count():fail(f"{name}: campaign and customization panels overlap")
    page.locator('#phaserCityHost').screenshot(path=str(vp_dir/'06-customization.png'))
    ...
    build_info=None
    if deployed:build_info=page.evaluate("async () => await (await fetch('build-info.json',{cache:'no-store'})).json()")
    context.close()
    return {'viewport':name,'size':[width,height],'fresh':fresh,'operating':operating,'seeded':seeded,'expansion':expansion,'buildInfo':build_info,'screenshots':7}

def main():
    p=argparse.ArgumentParser();p.add_argument('--url',required=True);p.add_argument('--out',default='artifacts/r14-clean');p.add_argument('--deployed',action='store_true');args=p.parse_args()
    ...
    print(json.dumps({'pass':report['pass'],'viewports':list(report['viewports']),'url':args.url,'deployed':args.deployed,'screenshots':sum(v['screenshots'] for v in report['viewports'].values())},indent=2))
