...
def visible_build_point(page):
    return page.evaluate("""()=>{const C=Codeopolis,s=C.phaserCity?.game?.scene?.getScene?.('CodeopolisCity'),w=C.game?.world,c=s?.cameras?.main,canvas=s?.game?.canvas;if(!s||!w||!c||!canvas)return null;const r=canvas.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2,candidates=[];for(let y=0;y<w.world.height;y++)for(let x=0;x<w.world.width;x++){const t=w.tile(x,y);if(!t||t.buildingId||t.occupiedBy||t.road)continue;const p=s.toWorld(x,y),px=r.left+(p.x-c.scrollX)*c.zoom,py=r.top+(p.y-c.scrollY)*c.zoom;if(px<r.left+8||px>r.right-8||py<r.top+8||py>r.bottom-8)continue;const top=document.elementFromPoint(px,py);if(top!==canvas)continue;candidates.push({x,y,px,py,d:(px-cx)*(px-cx)+(py-cy)*(py-cy)})}candidates.sort((a,b)=>a.d-b.d);return candidates[0]||null}""")
def manual_first_build(page):
    ...
    pt=visible_build_point(page)
    if not pt: fail(f"No visible hit-testable empty tile for first building · {diagnostics(page)}")
    page.mouse.click(pt['px'],pt['py']);page.wait_for_function("()=>Codeopolis.game.world.placedBuildings().length===1",timeout=8000)
