(function(C){
  ...
  class CompanyTeam{
    ...
    onStageCompleted(project,stage){const consulted=this.data.consultations.filter(x=>x.projectId===project.id&&x.stageIndex===project.stage-1),memberIds=[...new Set(consulted.map(x=>x.memberId))];for(const id of memberIds){const r=this.relation(id);r.trust=Math.min(100,r.trust+3);r.completedCollaborations++;r.lastProjectId=project.id}if(memberIds.length)C.events.emit('company:collaboration-earned',{project,memberIds})}
