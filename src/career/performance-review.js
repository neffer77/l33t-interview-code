(function(C){
  ...
  class PerformanceReview{
    ...
    ensure(){const old=this.state.companyPerformance||{};return this.state.companyPerformance={version:1,reviews:old.reviews||[],reviewedProjectCounts:old.reviewedProjectCounts||{},promotions:old.promotions||[]}}
    ...
    companyProjects(){const e=this.employment();return e?(this.projects.data.history||[]).filter(p=>p.companyId===e.companyId):[]}
    newProjects(){const e=this.employment();if(!e)return 0;return Math.max(0,this.companyProjects().length-(this.data.reviewedProjectCounts[e.companyId]||0))}
    eligible(){const e=this.employment();if(!e)return{ok:false,reason:'Accept a company offer first.'};const count=this.newProjects();return count>=3?{ok:true,count}:{ok:false,count,reason:`Complete ${3-count} more evidence-backed project${3-count===1?'':'s'} at ${e.companyName} before the next review.`}}
    evidence(){const recent=this.companyProjects().slice(0,this.newProjects()||3),delivery=recent.length?Math.round(recent.reduce((s,p)=>s+(p.score||0),0)/recent.length):0,types=new Set(recent.flatMap(p=>(p.evidence||[]).map(e=>e.type))),breadth=Math.min(100,Math.round(types.size/5*100)),trust=this.team.averageTrust(),readiness=this.game.phase16?.readiness?.score?.()||0;return{delivery,breadth,trust,readiness,projects:recent.length,types:[...types]}}
    review(){const gate=this.eligible();if(!gate.ok)return gate;const e=this.employment(),signals=this.evidence(),score=clamp(signals.delivery*.5+signals.readiness*.2+signals.trust*.15+signals.breadth*.15),promotionReady=score>=86&&signals.delivery>=82&&signals.readiness>=75&&signals.trust>=10&&this.level()<3,band=promotionReady?'Promotion Ready':score>=82?'Exceeds Expectations':score>=70?'Meets Expectations':'Needs Stronger Evidence';const rec={id:`review-${Date.now()}`,companyId:e.companyId,companyName:e.companyName,role:e.role,score,band,promotionReady,signals,projectCount:this.companyProjects().length,at:new Date().toISOString()};this.data.reviews.unshift(rec);this.data.reviews=this.data.reviews.slice(0,24);this.data.reviewedProjectCounts[e.companyId]=this.companyProjects().length;C.events.emit('company:performance-review',{review:rec,employment:e});return{ok:true,review:rec}}
    last(){const e=this.employment();return e?this.data.reviews.find(r=>r.companyId===e.companyId)||null:null}
