(function(C){
  ...
  class RecruitingSystem{
    ...
    finish(){const a=this.current();if(!a)return null;const company=this.company(a.companyId);const coding=a.problemIds.filter(id=>a.passed[id]).length;const comm=Object.values(a.communication).reduce((s,x)=>s+x,0);const score=Math.round((coding/a.problemIds.length)*75+(comm/(a.problemIds.length*2))*25);const passed=score>=70&&coding>=Math.ceil(a.problemIds.length*.67);a.status=passed?'passed':'failed';const rec={companyId:a.companyId,at:new Date().toISOString(),score,coding,total:a.problemIds.length,passed};this.data.history.unshift(rec);if(passed){this.data.offers=this.data.offers.filter(id=>id!==a.companyId);if(!this.data.completed.includes(a.companyId))this.data.completed.push(a.companyId);this.state.money=(this.state.money||0)+800+score*5;this.state.research=(this.state.research||0)+100;this.characters.award('marcus',18,`${company.name} interview completed`)}this.data.active=null;C.events.emit('story:interview-finished',{company,record:rec});return rec}
