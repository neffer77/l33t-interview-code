(function(C){
  ...
  function install(){if(install.done)return true;install.done=true;sync();addEventListener('resize',sync,{passive:true});window.visualViewport?.addEventListener?.('resize',sync,{passive:true});addEventListener('orientationchange',()=>setTimeout(sync,100),{passive:true});for(const evt of['civilization:phaser-ready','civilization:phaser-fallback','world:building-placed','world:building-unplaced','world:expanded','learning:resource-earned','r4:empty-land-ready','campaign:ready'])C.events?.on?.(evt,()=>setTimeout(sync,0));C.events?.emit?.('r14:player-acceptance-ready',{version:VERSION,viewports:VIEWPORTS});return true}
