(function(C){
  ...
  function cityViewportHealthy(hostRect,vp,mode){if(!hostRect)return true;if(mode==='phone_landscape')return hostRect.width>=vp.width*.9&&hostRect.height>=vp.height*.6;return hostRect.width>=280&&hostRect.height>=250}
  ...
  C.R14PlayerAcceptance={VERSION,VIEWPORTS,viewportSize,modeFor,visible,rect,firstRun,cityViewportHealthy,sync,audit,closeCompetingPanel,settleRenderer,install};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
