(function(C){
  ...
  class PhaserCityScene extends Phaser.Scene{
    constructor(){super({key:'CodeopolisCity'});this.tile=32;this.drag=null;this.pinch=null;this.ambient=[]}
    init(data){this.adapter=data.adapter;this.world=data.world;this.snapshot=this.adapter.snapshot()}
    create(){
      this.assets=C.Phase44Assets;this.tile=this.assets.tileSize||32;this.generatePixelTextures();
      this.cameras.main.setBackgroundColor('#183b39');this.renderWorld();
    ...
    generatePixelTextures(){
      ...
      for(const [name,p] of Object.entries(A.terrain))for(let v=0;v<(A.variants[name]||1);v++){
        if(name==='water'){
          for(let f=0;f<2;f++)this.texture(`terrain-water-${v}-${f}`,g=>{this.pixel(g,0,0,t,t,p.base);for(let y=5+v*3+f*2;y<t;y+=9){this.pixel(g,(v*7+y)%15,y,12,2,p.light,.72);this.pixel(g,(v*11+y+10)%22,y+3,8,1,p.dark,.7)}this.pixel(g,0,t-2,t,2,p.dark)});
          const anim=`water-${v}`;if(!this.anims.exists(anim))this.anims.create({key:anim,frames:[{key:`terrain-water-${v}-0`},{key:`terrain-water-${v}-1`}],frameRate:2,repeat:-1});
        }else this.texture(`terrain-${name}-${v}`,g=>{
          this.pixel(g,0,0,t,t,p.base);this.pixel(g,0,t-2,t,2,p.dark,.45);
          const dots=name==='grass'?7:name==='dirt'?10:5;for(let i=0;i<dots;i++){const x=(i*11+v*7+3)%29,y=(i*17+v*5+4)%27,size=(i+v)%3===0?2:1;this.pixel(g,x,y,size,size,i%2?p.light:p.dark,.72)}
          if(name==='grass'){this.pixel(g,6+v*3,7,1,4,p.light,.8);this.pixel(g,5+v*3,8,1,1,p.accent,.8)}
        });
      }
      ...
      for(let mask=0;mask<16;mask++)this.texture(`road-${mask}`,g=>{
        const R=A.roads;this.pixel(g,9,9,14,14,R.base);if(mask&1)this.pixel(g,9,0,14,16,R.base);if(mask&2)this.pixel(g,16,9,16,14,R.base);if(mask&4)this.pixel(g,9,16,14,16,R.base);if(mask&8)this.pixel(g,0,9,16,14,R.base);
        this.pixel(g,12,12,8,8,R.line,.22);if(mask&1)this.pixel(g,15,0,2,10,R.line,.45);if(mask&2)this.pixel(g,22,15,10,2,R.line,.45);if(mask&4)this.pixel(g,15,22,2,10,R.line,.45);if(mask&8)this.pixel(g,0,15,10,2,R.line,.45);
      });
    ...
    refresh(){this.snapshot=this.adapter.snapshot();this.tweens.killTweensOf(this.ambient);this.ambient=[];this.children.removeAll(true);this.renderWorld()}
    renderWorld(){
      const s=this.snapshot,t=this.tile,roadSet=new Set(s.roads.map(r=>`${r.x},${r.y}`));
      for(let y=0;y<s.height;y++)for(let x=0;x<s.width;x++){
        const terrain=s.terrain[y][x],count=this.assets.variants[terrain]||1,v=this.variant(x,y,count);
        if(terrain==='water'){const sprite=this.add.sprite(x*t+t/2,y*t+t/2,`terrain-water-${v}-0`).setDepth(0);sprite.play(`water-${v}`)}
        else this.add.image(x*t+t/2,y*t+t/2,`terrain-${terrain}-${v}`).setDepth(0);
        if(terrain==='forest'){
          const tree=this.add.image(x*t+t/2,y*t+t+4,`tree-${v%3}`).setOrigin(.5,1).setDepth(2+y*.001);this.ambient.push(tree);this.tweens.add({targets:tree,angle:{from:-1.2,to:1.2},duration:1800+v*250,yoyo:true,repeat:-1,ease:'Sine.easeInOut'});
        }
      }
      for(const r of s.roads)this.add.image(r.x*t+t/2,r.y*t+t/2,`road-${this.roadMask(r.x,r.y,roadSet)}`).setDepth(3);
      for(const b of s.buildings){const key=`building-${this.assets.buildings[b.district]?b.district:'core'}`,img=this.add.image(b.x*t+t/2,b.y*t+t+7,key).setOrigin(.5,1).setDepth(6+b.y*.01);if(b.progress<1){img.setAlpha(.45+.55*b.progress);const dust=this.add.rectangle(b.x*t+t/2,b.y*t+t/2,18,5,0xd9b26e,.38).setDepth(7);this.tweens.add({targets:dust,y:dust.y-8,alpha:0,duration:850,repeat:-1})}}
      if(s.selected)this.add.rectangle(s.selected.x*t+t/2,s.selected.y*t+t/2,t-2,t-2).setStrokeStyle(3,0xffdf72,1).setFillStyle(0xffffff,0).setDepth(20);
    }
  }
