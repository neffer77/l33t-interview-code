(function(C){
  ...
  class PhaserCityScene extends Phaser.Scene{
    ...
    /* One badge column per building. Every overlay module picked its own offset
       ...
       Callers reserve their own height rather than a slot number, because the
       markers are not all one line tall. Keyed by cell, so an overlay anchored
       on a tile rather than a building shares the column with it. */
    badgeY(b,height=11){
      ...
      const used=this.badgeSlots.get(key)||0;this.badgeSlots.set(key,used+height);
      return top-6-used}
    renderWorld(){
