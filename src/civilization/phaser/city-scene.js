(function(C){
  ...
  class PhaserCityScene extends Phaser.Scene{
    ...
    decorateTile(x,y,terrain,occupied){if(occupied||terrain==='water')return;const p=this.toWorld(x,y),roll=this.variant(x,y,1000,71)/1000,P=this.assets.props,d=this.iso.depth(x,y,8);if(terrain==='forest'||roll<.10){const tree=this.add.image(p.x,p.y+9,`tree-${this.variant(x,y,5,9)}`).setOrigin(.5,.92).setDepth(d+12);this.ambient.push(tree);return}if(roll<.18){const g=this.add.graphics().setDepth(d+3);g.fillStyle(P.flowerA,1);g.fillRect(p.x-7,p.y-4,2,2);g.fillStyle(P.flowerB,1);g.fillRect(p.x+5,p.y+3,2,2);g.fillStyle(0x497544,1);g.fillRect(p.x-6,p.y-2,1,3);this.ambient.push(g)}else if(roll<.24){const g=this.add.graphics().setDepth(d+3);g.fillStyle(P.stone,1);g.fillRect(p.x-5,p.y-2,8,4);g.fillStyle(P.stoneLight,.8);g.fillRect(p.x-3,p.y-3,4,2);this.ambient.push(g)}}
    ...
    renderWorld(){
      ...
      for(let y=0;y<s.height;y++)for(let x=0;x<s.width;x++){const terrain=s.terrain[y][x],v=this.variant(x,y,this.assets.variants[terrain]||1),p=this.toWorld(x,y),key=`terrain-${terrain}-${v}-${terrain==='water'?this.variant(x,y,2,33):0}`,img=this.add.image(p.x,p.y,key).setDepth(this.iso.depth(x,y,0));if(terrain==='water')this.tweens.add({targets:img,alpha:{from:.88,to:1},duration:1300+v*120,yoyo:true,repeat:-1});const occupied=!!this.world.tile(x,y)?.buildingId||!!this.world.tile(x,y)?.road;this.decorateTile(x,y,terrain,occupied)}
      for(const r of s.roads){const p=this.toWorld(r.x,r.y);this.add.image(p.x,p.y,`road-${r.mask||0}`).setDepth(this.iso.depth(r.x,r.y,4))}
      for(const b of s.buildings){const p=this.toWorld(b.x,b.y),key=b.known&&this.assets.buildings[b.district]?`building-${b.district}`:b.known?'building-core':'building-unknown',img=this.add.image(p.x,p.y+9,key).setOrigin(.5,.86).setDepth(this.iso.depth(b.x,b.y,30));img.setAlpha(.45+.55*b.progress);let dust=null;if(b.progress<1){dust=this.add.rectangle(p.x,p.y-2,24,6,0xd9b26e,.4).setDepth(this.iso.depth(b.x,b.y,40));this.tweens.add({targets:dust,y:dust.y-11,alpha:0,duration:850,repeat:-1})}this.buildingRefs.set(`${b.x},${b.y}`,{image:img,dust,progress:b.progress,id:b.id})}
