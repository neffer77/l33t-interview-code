(function(C){
  ...
  async function start(world,state){
    ...
    try{
      ...
      setTimeout(()=>{const p=C.phaserCity;if(!p?.game)return;const scene=p.game.scene.getScene('CodeopolisCity');if(!scene||!scene.sys?.isActive?.())fallback(new Error('Phaser City scene failed health check'));else C.Phase44Diagnostics?.installControls?.(host)},1800);
      C.events.emit('civilization:phaser-ready',{renderer:'phaser',version:Phaser.VERSION});return true;
