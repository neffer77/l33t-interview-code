(function(C){
  ...
  const LIFECYCLE_URL='src/civilization/building-lifecycle.js';
  const LIFECYCLE_UI_URL='src/civilization/building-lifecycle-ui.js';
  ...
  async function loadRuntime(){
    ...
    if(!C.BuildingLifecycle)await loadScript(LIFECYCLE_URL);
    C.BuildingLifecycle?.install?.();
    if(!C.BuildingLifecycleUI)await loadScript(LIFECYCLE_UI_URL);
    ...
    if(!window.Phaser||!C.PhaserCityScene||!C.Phase44Assets||!C.PlacementController||!C.BuildingCatalogUI||!C.BuildingLifecycleUI)throw new Error('Phaser city runtime failed to load');
  }
  function fallback(reason){
    ...
    C.phaserCity=null;
    console.warn('Phaser city unavailable; retaining Canvas2D renderer.',reason);
    C.events.emit('civilization:phaser-fallback',{error:String(reason||'unknown renderer failure')});
  }
  function setActive(active){
    const p=C.phaserCity;if(!p?.game)return;
    const scene=p.game.scene.getScene('CodeopolisCity');if(!scene)return;
    if(active){if(scene.scene.isSleeping())scene.scene.wake();if(scene.scene.isPaused())scene.scene.resume();p.game.loop.wake();p.resize?.();p.catalog?.render?.();p.lifecycle?.render?.()}
  ...
  async function start(world,state){
    ...
    try{
      ...
      const scene=game.scene.getScene('CodeopolisCity'),placement=new C.PlacementController(scene,world),catalog=new C.BuildingCatalogUI(host,world,state),lifecycle=new C.BuildingLifecycleUI(host,world,state);
      C.phaserCity={game,adapter,placement,catalog,lifecycle,host,legacyCanvas:canvas,active:true,resize:()=>{if(host.clientWidth&&host.clientHeight)game.scale.resize(host.clientWidth,host.clientHeight)},setActive};
      ...
      C.events.emit('civilization:phaser-ready',{renderer:'phaser',version:Phaser.VERSION,placement:true,catalog:true,lifecycle:true});return true;
