(function(C){
  ...
  class CivilizationWorldAdapter{
    ...
    terrainAt(x,y){
      ...
      if(tile?.buildingId)return'grass';
    ...
    diagnostics(){
      ...
      for(const [k,t] of Object.entries(w.tiles||{})){
        ...
        if(t?.road&&t?.buildingId)out.roadBuildingConflicts++;
    ...
    snapshot(now=Date.now()){
      ...
      return{version:2,width:w.width,height:w.height,seed:w.seed||1337,dayPhase:w.dayPhase||0,camera:{...w.camera},selected:w.selected?{...w.selected}:null,
