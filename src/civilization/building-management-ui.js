(function(C){
  ...
  class BuildingManagementUI{
    ...
    bind(){
      ...
      C.events.on('world:road-changed',()=>this.render());
    ...
    render(){const b=this.selected();if(!b||!this.el)return;const def=C.BuildingRegistry.definition(this.world,b.tile.buildingId),fx=this.world.buildingEffects(b.x,b.y),adj=this.world.adjacencyStatus?.(b.x,b.y),cost=this.world.upgradeCost(b.x,b.y),complete=this.world.constructionProgress(b.tile)>=1,max=fx.level>=fx.maxLevel;
      ...
      this.el.innerHTML=`<div class="p1-manager-head"><div><div class="p1-manager-title">${def.icon} ${def.name}</div><div class="p1-level-row">${Array.from({length:fx.maxLevel},(_,i)=>`<span class="p1-level-pip ${i<fx.level?'on':''}"></span>`).join('')}<b>Level ${fx.level}</b></div></div><button class="p1-manager-close" aria-label="Close">×</button></div><div class="p1-manager-effects">${effects.length?effects.map(e=>`<div class="p1-effect"><b>${e[0]}</b><div>${e[1]>0?'+':''}${e[1]}</div></div>`).join(''):'<div class="p1-effect">Civic utility structure</div>'}</div><div class="p1-adjacency-row"><span class="p1-adj-chip ${adj?.roadConnected?'on':''}">🛣️ ${adj?.roadConnected?'+10% road':'No road access'}</span><span class="p1-adj-chip ${adj?.sameDistrict?'on':''}">🏙️ ${adj?.sameDistrict?`+${Math.round(adj.districtBonus*100)}% district`:'No district cluster'}</span>${adj?`<span class="p1-adj-chip on">×${adj.multiplier.toFixed(2)} planning</span>`:''}</div><div class="p1-manager-upgrade">${!complete?'🏗️ Construction in progress':max?'⭐ Maximum tier reached':`Next tier: +50% base output · cost 💰 ${cost}`}</div><div class="p1-manager-actions"><button class="p1-upgrade" ${(!complete||max)?'disabled':''}>⬆️ ${max?'Max level':`Upgrade · ${cost}`}</button><button class="p1-move">↔️ Move</button><button class="p1-remove" ${b.tile.buildingId==='camp'?'disabled':''}>🚧 Demolish</button></div><div class="p1-manager-note">${def.footprint.w}×${def.footprint.h} footprint · ${def.district} district · Roads add 10%; each adjacent same-district building adds 5%, capped at 20%.</div>`;
