(function(C){
  ...
  class BuildingCatalogUI{
    ...
    bind(){
      ...
      C.events.on('world:tool',()=>this.render());
    ...
    acquire(id){
      const s=C.BuildingRegistry.status(this.world,this.state,id);if(s.locked){this.notify(`🔒 ${s.locked}`);return false}if(!s.owned){if(!s.affordable){this.notify(`Need 💰 ${s.def.cost}`);return false}this.state.money=Math.max(0,(this.state.money||0)-s.def.cost);this.state.buildings=this.state.buildings||[];this.state.buildings.push(id);try{typeof persist==='function'&&persist(false)}catch{}}
    ...
    render(){
      ...
      const list=C.BuildingRegistry.catalog(this.world,this.state);this.grid.innerHTML=list.map(s=>{const d=s.def,action=s.owned?`Place owned ×${s.owned}`:s.locked?'Locked':s.affordable?`Buy 💰 ${d.cost}`:`Need 💰 ${d.cost}`,disabled=!!s.locked||(!s.owned&&!s.affordable);return `<article class="p1-building-card ${disabled?'locked':''}"><div class="p1-building-title">${d.icon} ${d.name}</div><div class="p1-building-meta"><span class="p1-building-chip">${d.footprint.w}×${d.footprint.h}</span><span class="p1-building-chip">${d.district}</span>${d.cost?`<span class="p1-building-chip">💰 ${d.cost}</span>`:''}</div><div class="p1-building-desc">${d.desc||'Civilization infrastructure.'}</div>${s.locked?`<div class="p1-building-reason">🔒 ${s.locked}</div>`:''}<button class="p1-building-action" data-build-id="${d.id}" ${disabled?'disabled':''}>${action}</button></article>`}).join('')||'<div>No buildings available yet.</div>';
