(function(C){
  ...
  class BuildingCatalogUI{
    ...
    train(id,index=0){const r=C.LearningCityLoop?.train?.(this.state,this.world,id,index);if(!r?.ok){this.notify(`⚠️ ${r?.reason||'No training mission available'}`);return false}this.notify(`🎯 Training for ${r.contract?.building?.name||id}`);return true}
    render(){
      ...
      const list=C.BuildingRegistry.catalog(this.world,this.state);this.grid.innerHTML=list.map(s=>{const d=s.def,cost=s.economyCost||{money:d.cost,resources:{}},costText=C.MultiResourceEconomy?.format?.(cost)||`💰 ${d.cost}`,missing=C.MultiResourceEconomy?.formatMissing?.(this.state,cost)||'',action=s.owned?`Place owned ×${s.owned}`:s.locked?'Locked':s.affordable?`Build · ${costText}`:`Need ${missing}`,disabled=!!s.locked||(!s.owned&&!s.affordable),contract=C.LearningCityLoop?.contract?.(this.state,this.world,d.id),next=contract?.next,mission=next?.recommendation?.challenge,learning=s.locked&&next?`<div class="p1-building-learning"><b>Learning contract:</b> ${next.text}${mission?`<br>🎯 ${mission.title||mission.name||mission.id} · ${mission.diff||mission.difficulty||''}`:''}</div>`:'';return `<article class="p1-building-card ${disabled?'locked':''}"><div class="p1-building-title">${d.icon} ${d.name}</div><div class="p1-building-meta"><span class="p1-building-chip">${d.footprint.w}×${d.footprint.h}</span><span class="p1-building-chip">${d.district}</span></div><div class="p1-building-cost">${costText}</div><div class="p1-building-desc">${d.desc||'Civilization infrastructure.'}</div>${s.locked?`<div class="p1-building-reason">🔒 ${s.locked}</div>`:(!s.owned&&!s.affordable?`<div class="p1-building-reason">Missing ${missing}</div>`:'')}${learning}${s.locked&&mission?`<button class="p3-learning-action" data-train-build-id="${d.id}">🎯 Train to unlock</button>`:''}<button class="p1-building-action" data-build-id="${d.id}" ${disabled?'disabled':''}>${action}</button></article>`}).join('')||'<div>No buildings available yet.</div>';
      this.grid.querySelectorAll('[data-build-id]').forEach(b=>b.onclick=()=>this.acquire(b.dataset.buildId));this.grid.querySelectorAll('[data-train-build-id]').forEach(b=>b.onclick=()=>this.train(b.dataset.trainBuildId));
    }
  }
