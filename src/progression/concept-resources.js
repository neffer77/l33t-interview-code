(function(C){
  ...
  function ensure(state){const r=state.learningResources||(state.learningResources={version:1,balances:{},earned:{},history:[]});r.version=1;r.balances=r.balances||{};r.earned=r.earned||{};r.history=Array.isArray(r.history)?r.history.slice(-49):[];for(const id of Object.keys(RESOURCE_DEFS)){r.balances[id]=Math.max(0,Number(r.balances[id])||0);r.earned[id]=Math.max(r.balances[id],Number(r.earned[id])||0)}return r}
  function award(state,payload={}){const r=ensure(state),reward=rewardFor(payload),id=reward.resourceId;r.balances[id]+=reward.amount;r.earned[id]+=reward.amount;const entry={id,amount:reward.amount,concept:reward.concept,at:Date.now()};r.history.push(entry);if(r.history.length>50)r.history.shift();C.events?.emit?.('learning:resource-earned',{...reward,balance:r.balances[id],entry});return{...reward,balance:r.balances[id]}}
  ...
  C.ConceptResources={RESOURCE_DEFS,DISTRICT_MAP,conceptKey,rewardFor,ensure,award,balances,install};
})(window.Codeopolis);