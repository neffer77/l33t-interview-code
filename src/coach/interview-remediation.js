...
class InterviewRemediation{
 ...
 tasks.push(this.task('retest',`Re-test: ${day.company} Interview Day`,'Transfer only counts when the repaired signal survives a fresh end-to-end mock loop.',{target:day.bar,signalTarget:TARGET[weak]||75,sourceDayId:day.id,company:day.company,profileId:day.profileId}));
 ...
 onInterviewDay(e){const d=e.day,p=this.active();if(!p||d.id===p.sourceDayId)return false;const weakRound=d.rounds?.find(r=>r.type===p.weakSignal),repairedScore=weakRound?.score||0,overall=d.summary?.overall||0,qualified=overall>=d.bar&&repairedScore>=this.target(p.weakSignal);if(!qualified)return false;return this.mark(t=>t.type==='retest'&&t.profileId===d.profileId,overall,{dayId:d.id,verdict:d.summary?.verdict,repairedSignal:p.weakSignal,repairedScore})}
